...
import { updateListing, fetchListingById } from '../redux/listingSlice';
import ImageUploader from '../components/ImageUploader';
import { 
  ArrowLeft, MapPin, Tag, DollarSign, Home, Building2, 
  Warehouse, Loader2, Save, AlertCircle, CheckCircle,
  Compass, Bed, Bath, UtensilsCrossed, Car
} from 'lucide-react';
...
export default function EditProperty() {
  ...
  const { currentListing, loading: listingLoading } = useSelector((state) => state.listings);
  const { loading: updateLoading, error } = useSelector((state) => state.listings);
  ...
  useEffect(() => {
    if (currentListing && !loaded) {
      ...
      setFormData({
        ...
        size: listing.size != null ? listing.size : '',
        sizeUnit: listing.sizeUnit || 'Marla',
        price: listing.price != null ? listing.price : '',
        beds: listing.beds != null ? listing.beds : '',
        baths: listing.baths != null ? listing.baths : '',
        kitchens: listing.kitchens != null ? listing.kitchens : '',
        rentalIncome: listing.rentalIncome != null ? listing.rentalIncome : '',
        totalFloors: listing.totalFloors || '',
        roadWidth: listing.roadWidth || '',
        parkingCapacity: listing.parkingCapacity || '',
        powerBackup: listing.powerBackup || '',
        plotType: listing.plotType || 'Standard',
        dimensions: listing.dimensions || '',
        floorLevel: listing.floorLevel || '',
        viewType: listing.viewType || '',
        carParking: listing.carParking || '',
        mapUrl: listing.mapUrl || '',
        isInstallmentAvailable: listing.isInstallmentAvailable || false,
        downPayment: listing.installmentDetails?.downPayment || '',
        monthlyInstallment: listing.installmentDetails?.monthlyInstallment || '',
        durationMonths: listing.installmentDetails?.durationMonths || '',
        images: listing.images || [],
        features: listing.features || [],
      });
      setLoaded(true);
    }
  }, [currentListing, loaded]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const result = await dispatch(updateListing({ id, data: formData })).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 py-8 transition-colors duration-200">
      <Helmet>
        <title>Edit Property | EstateSocial Pakistan</title>
        <meta name="description" content="Update your property listing on EstateSocial." />
      </Helmet>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center space-x-3">
              <button 
                ...
                className="p-2 text-gray-500 hover:text-gray-700 dark:hover:text-zinc-200 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-xl"
              >
                <ArrowLeft className="h-5 w-5" />
              </button>
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-zinc-50">Edit Property Listing</h1>
                <p className="text-sm text-gray-500 dark:text-zinc-400 mt-0.5">
                  Update inventory details, pricing, photos, or commercial specifications
                </p>
              </div>
            </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-[#0B111E] rounded-2xl border border-gray-200/80 dark:border-zinc-800 overflow-hidden shadow-sm p-6 sm:p-8">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe construction, layout, and amenities..."
                  />
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-zinc-400">Select verified amenities and update property photos</p>
              </div>
              ...
              <div className="border-t border-gray-200 dark:border-zinc-800 pt-6">
                ...
                {formData.isInstallmentAvailable && (
                  <div className="grid md:grid-cols-3 gap-4 mt-4 p-4 bg-gray-50 dark:bg-zinc-900/50 rounded-xl border border-gray-200 dark:border-zinc-800">
                    <div>
                      <label htmlFor="downPayment" className="block text-xs font-semibold text-gray-700 dark:text-zinc-300 mb-1">
                        Down Payment (PKR) <span className="text-red-500">*</span>
                      </label>
                      <input
                        id="downPayment"
                        name="downPayment"
                    ...
                    <div>
                      <label htmlFor="monthlyInstallment" className="block text-xs font-semibold text-gray-700 dark:text-zinc-300 mb-1">
                        Monthly (PKR) <span className="text-red-500">*</span>
                      </label>
                      <input
                    ...
                    <div>
                      <label htmlFor="durationMonths" className="block text-xs font-semibold text-gray-700 dark:text-zinc-300 mb-1">
          ...
          <div className="px-6 py-4 bg-gray-50 dark:bg-zinc-900/50 border-t border-gray-200 dark:border-zinc-800 flex items-center justify-between mt-8 -mx-6 sm:-mx-8 -mb-6 sm:-mb-8">
            <button
              type="button"
              onClick={prevStep}
              disabled={step === 1}
              className="px-4 py-2.5 border border-gray-300 dark:border-zinc-700 text-gray-700 dark:text-zinc-300 rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all text-sm font-semibold"
            >
              Previous
            </button>
            ...
            <div className="flex items-center space-x-3">
              {step < 3 ? (
                <button
                  type="button"
                  onClick={nextStep}
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold transition-all text-sm flex items-center space-x-2 shadow-sm"
                >
                  <span>Next Step</span>
                </button>
              ) : (
                <button
                  ...
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-xl font-bold transition-all text-sm flex items-center space-x-2 shadow-sm"
                >
                  {saving ? (
                    <>
